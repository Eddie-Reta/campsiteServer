...
.post(authenticate.verifyUser, (req, res, next) => {
    ...
    .then(campsite => {
        if (campsite) {
            ...
            err = new Error(`Commetns ${req.params.campsiteId} not found.`);
            err.status = 404;
            return next(err);
        }
    })
...
.get((req, res, next) => {
    ...
    .then(campsite => {
        if (campsite && campsite.comments.id(req.params.commentId)) {
            ...
            err = new Error(`Commetns ${req.params.commentId} not found.`);
            err.status = 404;
            return next(err);
        }
    })
...
.post(authenticate.verifyUser, (req, res, next) => {
    res.statusCode = 403;
    res.end(`POST operstion not supported on /campsites/${req.params.commentId}/comments/${req.params.commentId}`);
})
